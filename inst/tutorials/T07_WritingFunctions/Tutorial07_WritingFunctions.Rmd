...
library(magrittr)
library(pheatmap)
gtex_median <- read_delim("GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct.gz", skip=2, delim="\t", col_types="ccnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn", na=c("", "NA", "na"))
...
The argument `skip=2` was required because the first two lines of the file contain information for specific software (an no consistent comment character was used).  

The code used to import the data is shown below.  This chunk cannot be run.  

```{r eval=FALSE, echo=TRUE}
gtex_median <- read_delim("GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct.gz", 
                          skip=2,
                          delim="\t",
                          col_types="ccnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn", 
                          na=c("", "NA", "na"))
```

Use `slice` to display the first ten rows of the **tibble** to perform a visual check.  Note, if you try to display too many rows, the tutorial may crash!  
...
For a large file, it can be important to examine the last rows of the data (use a negative integers with `slice`).  If the file is sorted in any way, unusual values may occur in these rows.  
...
**Escape characters** are used in computing to invoke an alternate intepretation for the character that follows, e.g. `\t` is not a small `t` but a `tab`.  **Escape characters** are analogous to the **control** key on your computer.  
